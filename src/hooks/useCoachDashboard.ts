...
async function fetchDashboard(userId: string): Promise<DashboardData> {
  ...
  if (!relations || relations.length === 0) {
    return { students: [], kpis: { activeStudents: 0, sessionsThisWeek: 0, avgAdherence: 0, alertCount: 0 }, feed: [] };
  }

  const studentIds = relations.map((r) => r.student_id);

  // 2-9. Parallel fetch all data
  const [
    profilesRes,
    programsRes,
    ...
    supabase.from("weekly_checkins").select("student_id, energy_level, sleep_quality, stress_level, muscle_soreness, week_start").in("student_id", studentIds).gte("week_start", wsStr).lte("week_start", weStr),
    supabase.from("session_swaps").select("student_id, id, created_at, original_day, new_day").in("student_id", studentIds).gte("created_at", weekStart.toISOString()).lte("created_at", weekEnd.toISOString()),
    supabase.from("external_sessions").select("student_id, id, activity_type, date").in("student_id", studentIds).gte("date", wsStr).lte("date", weStr),
  ]);
  ...
  const externals = externalsRes.data || [];

  // Program weeks + sessions (parallel)
  let programWeekMap = new Map<string, { total: number }>();
  let sessionsPerStudent = new Map<string, number>();

  if (programs.length > 0) {
    const programIds = programs.map((p) => p.id);
    const [weeksRes, firstWeeksRes] = await Promise.all([
      supabase.from("program_weeks").select("program_id, week_number").in("program_id", programIds),
      supabase.from("program_weeks").select("id, program_id, week_number").in("program_id", programIds).order("week_number"),
    ]);
    ...
    grouped.forEach((weekNums, progId) => {
      programWeekMap.set(progId, { total: weekNums.length });
    });

    // First week per program for session count
    const firstWeekByProgram = new Map<string, string>();
    (firstWeeksRes.data || []).forEach((w) => {
  ...
  const studentOverviews: StudentOverview[] = profiles.map((profile) => {
    ...
    const doneThisWeek = completedSessions.filter((cs) => cs.student_id === profile.user_id).length;
...
export const useCoachDashboard = () => {
  ...
  return {
    ...
    kpis: data?.kpis || { activeStudents: 0, sessionsThisWeek: 0, avgAdherence: 0, alertCount: 0 },
    feed: data?.feed || [],
